...
export async function extractSyllabusWithAI(text) {
...
export async function generateNotes(subjectName, topicTitle, sourceText) {
  const content = await callOpenAI([
    {
      role: "system",
      content:
...
export async function generateQuiz(subjectName, topicTitle, sourceText) {
  const content = await callOpenAI(
    [
      {
        role: "system",
...
function generateMockNotes(subject, topic, source) {
  const lines = source
    .split(/\n/)
    .map((l) => l.trim())
...
function generateMockQuiz(subject, topic) {
  return [
    {
      id: "q1",
      type: "mcq",
