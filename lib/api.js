...
import { uploadSyllabusToSupabase } from "./supabase";
...
export async function uploadSyllabus(file, text) {
  ...
  uploadSyllabusToSupabase(file?.name ?? "pasted-text", rawText).catch(() => {});

  return { success: true, subjects, extractedCount: extraction.subjects.length };
}

export async function generateStudyPlanApi(dailyMinutes) {
...
export async function markAttendance(subjectId, date, present) {
  return upsertAttendance(subjectId, date, present);
}

export async function setAttendanceTarget(attendanceTarget) {
  updateSettings({ attendanceTarget });
  return { success: true };
}

export async function fetchMarks(subjectId, targetGrade) {
...
export async function patchMarkComponent(id, obtainedMarks) {
...
export async function removeMarkComponent(id) {
  deleteMarkComponent(id);
  return { success: true };
}

export async function fetchNotes(subjectId) {
  const subjects = getSubjects();
