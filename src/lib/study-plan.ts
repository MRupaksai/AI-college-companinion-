import type { ExtractedSubject, StudyPlanDay } from "./types";
...
export function generateStudyPlan(
  ...
  const subjectExamMap = new Map(
    subjects.map((s) => [s.id, s.examDate])
  );
  ...
  while (taskIndex < allTasks.length && dayCount < maxDays) {
    ...
    const dayTasks: StudyPlanDay["tasks"] = [];
    ...
    if (dayTasks.length > 0) {
      ...
      plan.push({
        ...
        subjectId: allTasks.find((t) => t.topic === primarySubject.topic)?.subjectId ?? subjects[0].id,
        subjectName: allTasks.find((t) => t.topic === primarySubject.topic)?.subjectName ?? subjects[0].name,
        tasks: dayTasks,
      });
    }
...
export function parseSyllabusStructure(text: string): ExtractedSubject[] {
  const subjects: ExtractedSubject[] = [];
  const lines = text.split(/\r?\n/).map((l) => l.trim()).filter(Boolean);

  let currentSubject: ExtractedSubject | null = null;
  let currentUnit: ExtractedSubject["units"][0] | null = null;
    ...
    if (datePattern.test(line) && currentSubject) {
      ...
      if (dateMatch) {
        ...
        currentSubject.importantDates.push({
          ...
          type: /exam|test|final/i.test(line) ? "exam" : /assignment|submit|due/i.test(line) ? "assignment" : "other",
        });
      }
    }
  }
